...
def setup_pipeline(config: dict):
    ...
    if config["paths"]["lora_weights_path"]:
        pipe.load_lora_weights(config["paths"]["lora_weights_path"],weight_name="pytorch_lora_weights.safetensors")
...
def process_single_image(file, pipe, transform, gen_outpath, grid_outpath, mask_outpath, generator, config):
    ...
    input_semseg_image = semseg_image
    input_semseg_image = transform(input_semseg_image)
    input_semseg_image = input_semseg_image[0,:]
    input_semseg_image = input_semseg_image[None,:]
    
    # Image generation loop
    for i in range(config["num_generations"]):
        generated_image = pipe(config["prompt"], num_inference_steps=30, generator=generator, image=input_semseg_image,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Run ControlNet inference pipeline.")
