...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help=("weight of the LoRA matrices attributing to the original model's weights."),
    )
    parser.add_argument(
        "--use_dora",
        action="store_true",
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if args.center_crop:
            y1 = max(0, int(round((instance_image.height - args.resolution[1]) / 2.0)))
            x1 = max(0, int(round((instance_image.width - args.resolution[0]) / 2.0)))
            instance_image = self.train_crop(instance_image)
        else:
            y1, x1, h, w = self.train_crop.get_params(instance_image, (args.resolution[0], args.resolution[1]))
...
def main(args):
    ...
    def get_lora_config(rank, alpha, use_dora, target_modules):
        base_config = {
            "r": rank,
            "lora_alpha": alpha,
    ...
    unet_lora_config = get_lora_config(rank=args.rank,alpha=args.alpha, use_dora=args.use_dora, target_modules=unet_target_modules)
