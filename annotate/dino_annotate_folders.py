...
def detect_objects_and_save_yolo_format(image_folder, output_dir="output_yolo", class_string="a person. a pedestrian. a human.", box_threshold=0.25, text_threshold=0.1):
    """
    Detects objects in images using Grounding DINO, extracts bounding box
    ...
    Args:
        ...
        class_string (str): Text prompt for the object(s) to detect.
    """
    ...
    text_prompt = class_string  # The object(s) to detect
...
# --- CLI Usage ---
    import argparse
    parser = argparse.ArgumentParser(description="Detect objects in images using Grounding DINO and save YOLO-format labels.")
    parser.add_argument('--input_folder', type=str, required=True, help='Path to input image folder')
    parser.add_argument('--output_dir', type=str, required=True, help='Directory to save YOLO labels and annotated images')
    parser.add_argument('--class_string', type=str, required=True, help='Text prompt for the object(s) to detect (e.g., "a car.", "a person.")')
    parser.add_argument('--box_threshold', type=float, default=0.25, help='Box threshold for detection (default: 0.25)')
    parser.add_argument('--text_threshold', type=float, default=0.1, help='Text threshold for detection (default: 0.1)')
    args = parser.parse_args()

    detect_cars_and_save_yolo_format(
        args.input_folder,
